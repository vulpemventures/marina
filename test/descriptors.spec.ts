...
describe('evaluate', () => {
  it('should replace xpubs', () => {
    ...
    expect(result.redeemScript).toEqual(
...
describe('parser', () => {
  ...
  it('should parse eltr template', () => {
    const text =
      'eltr(c6047f9441ed7d6d3045406e95c07cd85c778e4b8cef3ca7abac09b95c709ee5,{raw(fff97bd5755eeea420453a14355235d382f6472f8568a18b2f057a1460297556),raw(e493dbf1c10d80f3581e4904930b1404cc6c13900ee0758474fa94abe8c4cd13)})';
    const [tree, remainingText] = parseScript(text);
    ...
    expect(res.redeemScript.toString('hex')).toEqual(
      '51200ef0975c80e0ff6cc3ef4c02944329bb9cea26c54db74ff1919bb3dfb75b980f'
    );
    expect(res.getWitnesses).toBeDefined();
    expect(
      res.getWitnesses!('fff97bd5755eeea420453a14355235d382f6472f8568a18b2f057a1460297556').map(
        (b) => b.toString('hex')
      )
    ).toEqual([
    ...
    expect(
      res.getWitnesses!('e493dbf1c10d80f3581e4904930b1404cc6c13900ee0758474fa94abe8c4cd13').map(
        (b) => b.toString('hex')
      )
    ).toEqual([
  ...
  it('should parse asm template', () => {
    const text =
      'asm(c6047f9441ed7d6d3045406e95c07cd85c778e4b8cef3ca7abac09b95c709ee5 OP_CHECKSIG)';
    const [tree, remainingText] = parseScript(text);
    expect(tree).toEqual({
      type: NodeType.ASM,
      value: script
        .fromASM('c6047f9441ed7d6d3045406e95c07cd85c778e4b8cef3ca7abac09b95c709ee5 OP_CHECKSIG')
        .toString('hex'),
      children: [],
    });
    expect(remainingText).toEqual('');
