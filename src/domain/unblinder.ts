...
import { AppRepository, AssetRepository, WalletRepository } from '../infrastructure/repository';
import { UnblindingData } from './transaction';
...
export class WalletRepositoryUnblinder implements Unblinder {
    ...
    constructor(private cache: WalletRepository, private appRepository: AppRepository, private assetRepository: AssetRepository, zkpLib: ZKPInterface) {
        ...
        for (const asset of assetSet) {
            ...
            try {
                const {
                    ...
                    ticker,
                    precision,
                } = await axios.get<any, AssetAxiosResponse>(`${webExplorerURL}/api/asset/${asset}`)
                    .then((r) => r.data)
