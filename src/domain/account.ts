...
import { AppRepository, WalletRepository } from "../infrastructure/repository";
...
export class AccountFactory {
  ...
  static async create(walletRepository: WalletRepository, appRepository: AppRepository, networks?: NetworkString[]): Promise<AccountFactory> {
...
export class Account {
  ...
  async deriveBatch(start: number, end: number, isInternal: boolean, updateCache = true): Promise<Buffer[]> {
    ...
    if (this.walletRepository && updateCache) {
      ...
      await this.walletRepository.updateScriptDetails(Object.fromEntries(scripts.map(script => [script.toString('hex'), {
  ...
  async getAllAddresses(): Promise<string[]> {
    ...
    const addresses = scripts.map(script => {
      ...
      return payments.p2wpkh({ output: script, network: this.network, blindkey: publicKey }).address!;
    }
    );
    return addresses;
  }
  ...
  async getNextAddress(isInternal: boolean): Promise<string> {
    ...
    const { [this.name]: accountDetails } = await this.walletRepository.getAccountDetails(this.name);
    if (!accountDetails) throw new Error('Account not found');
    const lastUsed = isInternal ? accountDetails.lastUsedInternalIndex : accountDetails.lastUsedExternalIndex;
  ...
  async sync(gapLimit = GAP_LIMIT): Promise<{
    ...
    for (const i of walletChains) {
      ...
      while (unusedScriptCounter <= gapLimit) {
        const scripts = await this.deriveBatch(batchCount, batchCount + gapLimit, isInternal, false);
    ...
    await Promise.allSettled([
      ...
      this.walletRepository.updateTxDetails(Object.fromEntries(Array.from(historyTxsId).map(txid => [txid, { height: txidHeight.get(txid) }]))),
    ]);
    // fetch the unspents
    ...
    await this.walletRepository.updateScriptUnspents(Object.fromEntries(unspents
    ...
    for (const script of scripts) {
      await this.chainSource.subscribeScriptStatus(script, async (_: string, status: string | null) => {
        ...
        await Promise.all([
          ...
          this.walletRepository.updateTxDetails(Object.fromEntries(history[0].map(({ tx_hash, height }) => [tx_hash, { height }]))),
        ]);

        const unspents = await this.chainSource.fetchUnspentOutputs([script]);
        const unspentForScript = unspents[0];
        await this.walletRepository.updateScriptUnspents({ [script.toString('hex')]: unspentForScript });
      });
    }
  }
  ...
  async subscribeAllScripts(): Promise<void> {
    ...
    for (const i of walletChains) {
      const isInternal = i === 1;
      let lastUsedIndex = isInternal ? lastUsed.internal : lastUsed.external;
