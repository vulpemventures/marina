...
import { ElectrumWS, ElectrumWSEvent } from "../../domain/ws/ws-electrs";
import { AppStatus } from "../../domain/app";
import { ChainSource, WsElectrumChainSource } from "../../domain/chainsource";
import { BlockstreamExplorerURLs, BlockstreamTestnetExplorerURLs, NigiriDefaultExplorerURLs } from "../../domain/explorer";
...
export class AppStorageAPI implements AppRepository {
    ...
    async getStatus(): Promise<AppStatus> {
        const statusValues = await Browser.storage.local.get([AppStorageKeys.ONBOARDING_COMPLETED, AppStorageKeys.AUTHENTICATED, AppStorageKeys.VERIFIED_MNEMONIC]);
        return {
    ...
    async getNetwork(): MaybeNull<NetworkString> {
        const { [AppStorageKeys.NETWORK]: value } = await Browser.storage.local.get(AppStorageKeys.NETWORK);
    ...
    async getEnabledSites(): Promise<string[]> {
        const { [AppStorageKeys.ENABLED_SITES]: value } = await Browser.storage.local.get(AppStorageKeys.ENABLED_SITES);
        return value ? value : [];
    }

    async enableSite(url: string): Promise<void> {
        console.log('site enabled', url);
    ...
    async disableSite(url: string): Promise<void> {
        ...
        if (index !== -1) return;
        return Browser.storage.local.set({ [AppStorageKeys.ENABLED_SITES]: enabledSites.splice(index, 1) });
    }
