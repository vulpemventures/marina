...
export class OnboardingStorageAPI implements OnboardingRepository {
    async getOnboardingMnemonic(): Promise<string> {
        const { [OnboardingStorageKeys.ONBOARDING_MNEMONIC]: mnemonic } = await Browser.storage.local.get(
    ...
    async getOnboardingPassword(): Promise<string> {
        const { [OnboardingStorageKeys.ONBOARDING_PASSWORD]: password } = await Browser.storage.local.get(
