...
import { TxDetails, UnblindingData, UnblindedOutput, CoinSelection } from "../../domain/transaction";
...
export const OutpointBlindingDataKey = new DynamicStorageKey<[txid: string, vout: number]>('blindingdata');
...
export class WalletStorageAPI implements WalletRepository {
    ...
    async getAccountDetails(...names: string[]): Promise<Record<string, AccountDetails>> {
        ...
        return Object.fromEntries(
            Object.entries(details)
                .map(([key, details]) => [AccountKey.decode(key)[0], details])
    ...
    async getLastUsedIndexes(): MaybeNull<{ internal?: number; external?: number; }> {
        const indexes = await Browser.storage.local.get([WalletStorageKey.INTERNAL_INDEX, WalletStorageKey.EXTERNAL_INDEX]);
        return {
    ...
    async updateTxDetails(txIDtoDetails: Record<string, TxDetails>): Promise<void> {
        ...
        return Browser.storage.local.set(Object.fromEntries(
            Object.entries(txIDtoDetails)
                .map(([txid, details]) => [TxDetailsKey.make(txid), { ...detailsInStorage[TxDetailsKey.make(txid)], ...details }])
        )
        );
    }

    async updateScriptDetails(scriptToDetails: Record<string, ScriptDetails>): Promise<void> {
        return Browser.storage.local.set(Object.fromEntries(Object.entries(scriptToDetails).map(([script, details]) => [ScriptDetailsKey.make(script), details])))
    }

    updateScriptUnspents(scriptToUnspents: Record<string, ListUnspentResponse>): Promise<void> {
        return Browser.storage.local.set(Object.fromEntries(
            Object.entries(scriptToUnspents)
                .map(([script, unspents]) => [ScriptUnspentsKey.make(script), unspents])
        ));
    }

    updateOutpointBlindingData(outpointToBlindingData: Array<[{ txID: string; vout: number; }, UnblindingData]>): Promise<void> {
        return Browser.storage.local.set(Object.fromEntries(
            outpointToBlindingData.map(([outpoint, blindingData]) => [OutpointBlindingDataKey.make(outpoint.txID, outpoint.vout), blindingData])
        ));
    }

    async getOutputBlindingData(txID: string, vout: number): Promise<{ txID: string, vout: number, blindingData?: UnblindingData }> {
    ...
    async getUtxos(network: NetworkString, ...accountNames: string[]): Promise<UnblindedOutput[]> {
        ...
        if (accountNames.length > 0) {
            ...
            scripts = scripts.filter(
                script => scriptDetails[script] && accountNames.includes(scriptDetails[script].accountName)
            )
        }

        const keys = scripts.map(s => ScriptUnspentsKey.make(s));
        const scriptToUnspent: Record<string, ListUnspentResponse> = await Browser.storage.local.get(keys);
        const outpoints = Object.values(scriptToUnspent).flat();
        const outpointsKeys = outpoints.map(outpoint => OutpointBlindingDataKey.make(outpoint.tx_hash, outpoint.tx_pos));
        const outpointsToBlindingData: Record<string, UnblindingData> = await Browser.storage.local.get(outpointsKeys);
        const allUtxos = outpoints.map(outpoint => ({
            txID: outpoint.tx_hash,
            vout: outpoint.tx_pos,
            blindingData: outpointsToBlindingData[OutpointBlindingDataKey.make(outpoint.tx_hash, outpoint.tx_pos)],
        }));

        const lockedOutpoints = await this.getLockedOutpoints();
        return allUtxos.filter(utxo => !lockedOutpoints.includes({ txID: utxo.txID, vout: utxo.vout }));
    }

    async selectUtxos(network: NetworkString, targets: { amount: number, asset: string }[], lock = false, ...acountNames: string[]): Promise<CoinSelection> {
        ...
        for (const target of targets) {
            ...
            const { inputs, outputs } = coinselect(
                utxos.map(utxo => ({ txId: utxo.txID, vout: utxo.vout, value: utxo.blindingData?.value })),
            ...
            if (inputs) {
                selectedUtxos.push(
                    ...(inputs as { txId: string, vout: number }[]).map(input => onlyWithUnblindingData
                        .find(utxo => utxo.txID === input.txId && utxo.vout === input.vout) as UnblindedOutput)
                );
            }
            ...
            if (outputs) {
                outputs
                    ...
                    .push(...(outputs as { value: number }[]).map(output => ({
                        asset: target.asset,
                        amount: output.value,
                    })));
            }
    ...
    async getEncryptedMnemonic(): Promise<string | undefined> {
        const { [WalletStorageKey.ENCRYPTED_MNEMONIC]: value } = await Browser.storage.local.get(WalletStorageKey.ENCRYPTED_MNEMONIC);
        return value ? value : undefined;
    }

    async getPasswordHash(): Promise<string | undefined> {
        const { [WalletStorageKey.PASSWORD_HASH]: value } = await Browser.storage.local.get(WalletStorageKey.PASSWORD_HASH);
        return value ? value : undefined;
    }

    async getMasterBlindingKey(): Promise<string | undefined> {
        const { [WalletStorageKey.MASTER_BLINDING_KEY]: value } = await Browser.storage.local.get(WalletStorageKey.MASTER_BLINDING_KEY);
        return value ? value : undefined;
    }

    async setSeedData(encryptedMnemonic: string, passwordHash: string, masterBlindingKey: string): Promise<void> {
    ...
    onNewUtxo(callback: (utxo: UnblindedOutput) => Promise<void>) {
        return Browser.storage.onChanged.addListener(async (changes, areaName) => {
            if (areaName !== "local") return;
            const listUnspentKeys = Object.entries(changes)
                .filter(([key, changes]) => ScriptUnspentsKey.is(key) && changes.newValue !== undefined)

            for (const [_, change] of listUnspentKeys) {
    ...
    onDeleteUtxo(callback: (utxo: UnblindedOutput) => Promise<void>): void {
        return Browser.storage.onChanged.addListener(async (changes, areaName) => {
            ...
            for (const [key, change] of listUnspentKeys) {
                // check if there is NEW utxo
    ...
    private async getScripts(...networks: NetworkString[]): Promise<Array<string>> {
        ...
        return Object.entries(wholeStorage)
            .filter(([key, value]) => ScriptDetailsKey.is(key) && networks.includes((value as ScriptDetails).network))
    ...
    private async lockOutpoints(outpoints: Array<{ txID: string, vout: number }>): Promise<void> {
        const { [WalletStorageKey.LOCKED_OUTPOINTS]: lockedOutpoints } = await Browser.storage.local.get(WalletStorageKey.LOCKED_OUTPOINTS);
        const current = lockedOutpoints ?? [];
        const until = Date.now() + WalletStorageAPI.LOCKTIME;
        for (const outpoint of outpoints) {
            if (current.findIndex((o: any) => o.txID === outpoint.txID && o.vout === outpoint.vout) === -1) {
    ...
    private async getLockedOutpoints(): Promise<Array<{ txID: string, vout: number }>> {
        try { await this.unlockOutpoints(); } catch (e) { /* ignore */ }
        const { [WalletStorageKey.LOCKED_OUTPOINTS]: lockedOutpoints } = await Browser.storage.local.get(WalletStorageKey.LOCKED_OUTPOINTS);
        return (lockedOutpoints ?? []).map((outpoint: LockedOutpoint) => ({ txID: outpoint.txID, vout: outpoint.vout }));
    }
}
