...
import { AppRepository, AssetRepository, TaxiRepository } from '../repository';
...
export class TaxiStorageAPI implements TaxiRepository {
    ...
    async getTaxiAssets(network?: NetworkString): Promise<(Asset | string)[]> {
        ...
        const result = await Promise.allSettled(hashes.map(hash => this.assetRepository.getAsset(hash)));
