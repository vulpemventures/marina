...
export type EnabledMarinaEvent = MarinaEvent<{ network: NetworkType; hostname: string }>;
export type DisabledMarinaEvent = MarinaEvent<{ network: NetworkType; hostname: string }>;
export type NetworkMarinaEvent = MarinaEvent<NetworkType>;
...
export function compareEnabledWebsites(
  oldState: Record<NetworkType, string[]>,
  newState: Record<NetworkType, string[]>,
  currentHostname: string
) {
  const events: (DisabledMarinaEvent | EnabledMarinaEvent)[] = [];

  for (const network of ['liquid', 'regtest'] as NetworkType[]) {
